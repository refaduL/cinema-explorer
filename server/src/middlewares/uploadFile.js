...
const { UPLOAD_MOVIE_POSTER_DIRECTORY, ALLOWED_FILE_TYPES, MAX_FILE_SIZE } = require("../config");
...
const storage_forString = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, UPLOAD_MOVIE_POSTER_DIRECTORY);
  },
  filename: function (req, file, cb) {
    const extName = path.extname(file.originalname);
    const fileName = file.originalname.replace(extName, "") + extName;
    cb(null, fileName);

    //1. const fileName = Date.now() + "-" + file.originalname.replace(extName, "") + extName;
    //2. const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
